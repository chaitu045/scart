...
class ISC_ADMIN_LOGS extends ISC_ADMIN_BASE
{
	...
	public function HandleToDo($Do)
	{
		switch(isc_strtolower($Do)) {
			case "administratorloggrid":
				if ($this->auth->HasPermission(AUTH_Manage_Logs) && gzte11(ISC_LARGEPRINT)) {
				...
				break;
			case "systemloggrid":
				if ($this->auth->HasPermission(AUTH_Manage_Logs)) {
	...
	private function SystemLogGrid()
	{
		...
		if(isset($_REQUEST['logsummary']) && $_REQUEST['logsummary'] != '') {
			...
			$sortURL .= sprintf("&logsummary=%s", $_REQUEST['logsummary']);
			$GLOBALS['HideClearResults'] = '';
