<?php
	...
	class ISC_ADMIN_AUTH
	{
		...
		public function HandleSTSToDo($ToDo)
		{
			...
			else if (is_numeric(isc_strpos($do, "googleps"))) {
				$GLOBALS['ISC_CLASS_ADMIN_GOOGLEPS'] = GetClass('ISC_ADMIN_GOOGLEPS');
				$GLOBALS['ISC_CLASS_ADMIN_GOOGLEPS']->HandleToDo($ToDo);
			}
		...
		*
		* @param array $check an array of AUTH_* constants to check (or a single value)
		* @param array $permissions optional list of permissions to check against, otherwise will use the current user
		*/
		public function hasAnyPermission ($check, $permissions = null)
		{
			if ($permissions === null) {
			...
			if (!is_array($check)) {
				// quick in_array if $check is not an array
				return in_array($check, $permissions);
			}

			// this statement is true if any value of $check exists in $permissions
			return array_diff($check, $permissions) !== $check;
		}
	}
